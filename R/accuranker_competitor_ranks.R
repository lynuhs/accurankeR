...
accuranker_competitor_ranks <- function (domain_name, date_range = "TODAY", silent = FALSE, simple_token = NULL){
  ...
  if (!(domain_name %in% domain_list$domainName)) {
    stop(paste0(domain_name, " does not exist in your authorized Accuranker account!"))
  }
  if (is.character(date_range)) {
    if (toupper(date_range) == "TODAY") {
      ...
      stop("date_range must contain two dates in the following format: c(YYYY-MM-DD, YYYY-MM-DD) or one of c('TODAY', 'YESTERDAY')")
    }
  }
    ...
    stop("date_range must contain two dates in the following format: c(YYYY-MM-DD, YYYY-MM-DD)")
  }
  ...
  if (loops > 1 & !silent) {
    cat(crayon::red(paste0("Splitting API calls into ", 
                           loops, " fetches\n")))
  }
  rankings <- NULL
  tryCatch({
    for (l in 1:loops) {
      startDate <- dates[1 + (days * (l - 1))]
      if (l == loops) {
        endDate <- dates[length(dates)]
      }
      else {
        endDate <- dates[days * l]
      }
      if (!silent) {
        if (loops > 1) {
          cat(crayon::red(paste0("Fetching data from Accuranker for API call ", 
                                 l, " of ", loops, "\n")))
        }
        else {
      ...
      request <- paste0("domains/", domain_list[which(domain_list$domainName == 
                                                        domain_name), "domainId"], "/keywords/?fields=", 
                        "id,", "competitor_ranks,", "&period_from=", 
                        startDate, "&period_to=", endDate)
      ...
      for (k in 1:(length(data))) {
        ...
        if (length > 0) {
          for (r in 1:length) {
            single_rank <- plyr::rbind.fill(single_rank, 
                                            listToDataFrame(data[[k]]$competitor_ranks[[r]]))
          }
          rankings <- plyr::rbind.fill(rankings, cbind(data.frame(keyword_id = data[[k]]$id, 
                                                                  stringsAsFactors = FALSE), single_rank))
        }
      }
    }
    rm(data)
    if (!silent) {
      cat(crayon::green("Competitor keyword rankings downloaded successfully!\n"))
    }
    return(rankings)
  }, error = function(e) {
