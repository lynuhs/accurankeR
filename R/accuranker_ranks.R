#' Get the keyword rankings for a specific domain
#'
...
accuranker_ranks <- function(domain_name, date_range = "TODAY", silent = FALSE, simple_token = NULL) {
  ...
  tryCatch({
    for (l in 1:loops) {
      ...
      for (k in 1:(length(data))) {
        single_rank <- NULL
        length <- length(data[[k]]$ranks)

        if(length > 0){
          for(r in 1:length){
            single_rank <- plyr::rbind.fill(
              single_rank,
              listToDataFrame(data[[k]]$ranks[[r]])
            )
          }
    ...
    if(!silent){cat(crayon::green("Keyword rankings downloaded successfully!\n"))}
    return(rankings)
  }, error = function(e) {
